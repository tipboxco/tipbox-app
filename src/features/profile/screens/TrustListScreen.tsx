...
import { Box, Text, VStack, HStack, Avatar, Input, InputField, Center } from '@gluestack-ui/themed';
...
const TrustCard: React.FC<{ user: TrustUser }> = ({ user }) => {
      ...
      <ScrollView>
        <VStack space="sm" p="$4">
          ...
          <Box flexDirection="row" flexWrap="wrap" justifyContent="flex-start">
            {users.map((user) => (
              <TrustCard key={user.id} user={user} />
            ))}
          </Box>
        </VStack>
      </ScrollView>
