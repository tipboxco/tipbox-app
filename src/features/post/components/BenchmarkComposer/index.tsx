        ...
        {[
          ...
          return (
            <Pressable
              ...
              flex={1}
              onPress={() =>
                setValue('selectedChoice', key, { shouldValidate: true })
              }
            >
              <Box
                h={44}
 ...
 */
export const BenchmarkComposer = forwardRef<
  BenchmarkComposerHandle,
...
>(({ productContext, onStateChange }, ref) => {
  ...
  useEffect(() => {
    ...
    const productName =
      nameParts.length > 1 ? nameParts.slice(1).join(' ') : productContext.name;
    setValue(
      'selectedProduct1',
      {
  ...
  useEffect(() => {
    if (compareProduct) {
      ...
      if (watch('selectedChoice') === 'product2')
        setValue('selectedChoice', undefined as any, { shouldValidate: true });
    }
  ...
  const onSubmit: SubmitHandler<BenchmarkPostFormData> = async data => {
    ...
    try {
      await createBenchmarkPostMutation.mutateAsync({
        ...
        description,
        // Backend en az 2 ürünün isSelected:true olmasını ister (karşılaştırmaya dahil olanlar).
        products: [
  ...
  useImperativeHandle(ref, () => ({
    submit: () => {
      const submitHandler = handleSubmit as unknown as (
        cb: SubmitHandler<BenchmarkPostFormData>
      ) => () => void;
