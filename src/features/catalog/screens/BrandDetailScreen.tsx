...
import type {
  ...
  TipsCategory,
  TipsProduct,
} from '@/src/types/TipsAndTricksCard';
import type {
  QuestionCardData,
  QuestionCardCategory,
  QuestionCardProduct,
} from '@/src/types/QuestionCard';
import type {
  ExperiencePostCardData,
  ExperiencePostCardContentItem,
} from '@/src/types/ExperienceCard';
...
type BrandDetailScreenNavigationProp = NativeStackNavigationProp<
  BrandStackParamList,
  'BrandDetailScreen'
>;
type BrandDetailScreenRouteProp = RouteProp<
  BrandStackParamList,
  'BrandDetailScreen'
>;
...
const BrandDetailScreen: React.FC = () => {
  ...
  const isJoinLeavePending =
    joinBrandMutation.isPending || leaveBrandMutation.isPending;
  ...
  const handleSharePress = useCallback(async () => {
    ...
    try {
      await Share.share({
        message: t('brandDetail.shareMessage', {
  ...
  const mapBrandPostToPostCardData = useCallback(
    (post: BrandFeedPost): PostCardData => {
      ...
      const postData =
        post.data as import('@/src/features/profile/types').ProfilePost;
      ...
      return {
        ...
        user: {
          ...
          avatar:
            avatarSource || require('@/assets/avatar/default-useravatar.png'),
        },
        ...
        images:
          postData.images
            ...
            .filter(
              (imgSource: any): imgSource is NonNullable<typeof imgSource> =>
                !!imgSource
            ) || [],
        stats: {
  ...
  const mapExperienceToCardData = useCallback(
    (item: BrandFeedPost): ExperiencePostCardData => {
      ...
      const postData =
        item.data as import('@/src/types/ExperienceCard').ExperiencePostApiItem;
      const avatarSource = toImageSource(postData.user.avatar)!;
      ...
      const contentBlocks =
        postData.experienceContent ??
        (Array.isArray(postData.content) ? postData.content : []);
      const content: ExperiencePostCardContentItem[] = Array.isArray(
        contentBlocks
      )
        ? contentBlocks.map(contentItem => {
            ...
            const stars =
              ratingVal <= 5
                ? Math.min(5, Math.max(0, Math.round(ratingVal)))
                : Math.floor(ratingVal / 20);
            return {
              tag: {
                icon:
                  contentItem?.title?.toLowerCase?.().includes('product') ||
                  contentItem?.title?.toLowerCase?.().includes('usage')
                    ? ('package' as const)
                    : ('tag' as const),
      ...
      if (rawProduct && (rawProduct as any).productId)
        contextDataId = (rawProduct as any).productId;
      const subNameRaw = rawProduct?.subName ?? '';
      const subName =
        subNameRaw && !/^Status:\s*(tested|own)$/i.test(String(subNameRaw))
          ? subNameRaw
          : '';
      ...
      return {
        ...
        contextData: {
          ...
          subName,
          image:
            productImage ?? require('@/assets/defaultImages/default-post.png'),
        ...
        images: (() => {
          ...
          return mapped.filter(
            (img: any) =>
              !isSameImageSource(img, productImage ?? defaultPostImage)
          );
        })(),
  ...
  const mapBenchmarkToCardData = useCallback(
    (item: BrandFeedPost): BenchmarkCardData => {
      ...
      const postData =
        item.data as import('@/src/types/BenchmarkCard').BenchmarkApiItem;
      const avatarSource = toImageSource(postData.user.avatar)!;
  ...
  const mapTipsToCardData = useCallback((item: BrandFeedPost): TipsCardData => {
    ...
    const postData =
      item.data as import('@/src/types/TipsAndTricksCard').TipsApiItem;
    const avatarSource = toImageSource(postData.user.avatar)!;
  ...
  const mapQuestionToCardData = useCallback(
    (item: BrandFeedPost): QuestionCardData => {
      ...
      const postData =
        item.data as import('@/src/types/QuestionCard').QuestionApiItem;
      const avatarSource = toImageSource(postData.user.avatar)!;
      ...
      const images =
        mappedImages.length > 0 ? mappedImages : [defaultPostImage];
    ...
    if (allPosts.length > 0) {
      console.log('📝 [BrandDetailScreen] All Posts:', {
        ...
        posts: allPosts.map(post => ({
          ...
          content:
            typeof post.data.content === 'string'
  ...
  const renderFeedItem = useCallback(
    (item: BrandFeedPost) => {
      console.log(
        item.type === CardType.EXPERIENCE
          ? 'Experience Rednder Edildi.'
          : 'Düz Card'
      );
      ...
      switch (item.type) {
        ...
        case 'question':
          ...
          if (
            'contextType' in item.data &&
            'contextData' in item.data &&
            'isBoosted' in item.data
          ) {
            return (
              <QuestionPostCard
                key={item.data.id}
    ...
    [
      mapBrandPostToPostCardData,
      mapExperienceToCardData,
      mapBenchmarkToCardData,
      mapQuestionToCardData,
      mapTipsToCardData,
    ]
  );
        ...
        <Box flex={1} bg={isDark ? '$backgroundDark950' : '$backgroundLight0'}>
          ...
          <Box flex={1} justifyContent='center' alignItems='center' px='$4'>
            <Text color='#CE4A4A' fontSize='$sm' textAlign='center'>
              {brandCatalogError
                ? `${t('brandDetail.error')}: ${brandCatalogError.message}`
                : t('brandDetail.brandNotFound')}
            </Text>
          </Box>
        </Box>
      </View>
          ...
          <HStack flex={1} alignItems='center' space='sm'>
            ...
            <VStack flex={1}>
              ...
              <HStack alignItems='center' space='sm'>
                ...
                <Text color='#9D9D9D' fontSize='$xs' fontWeight='$medium'>
                  {t('brandDetail.followers', {
                    count: brandCatalog?.followers,
                  })}
                </Text>
              </HStack>
            </VStack>
      ...
      >
        ...
        <Animated.View style={bannerAnimatedStyle}>
          ...
          >
            <Image
              source={
                toImageSource(brandCatalog.bannerImage) ||
            ...
            >
              ...
              <Text color='#FFFFFF' fontSize='$2xs' lineHeight='$sm' flex={1}>
                {t('brandDetail.discoverExperiences', {
                  brandName: brandCatalog.name,
                })}
              </Text>
            </HStack>
              ...
              >
                <VStack space='sm' flex={1}>
                  ...
                  <Button
                    ...
                    onPress={() => {
                      if (brandId) {
                        navigation.navigate('BrandProductBookScreen', {
                          brandId,
                        });
                      }
                    }}
